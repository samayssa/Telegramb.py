...
async def start_auction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if START_IMAGE_URL:
        try:
            # FIXED → Added reply_markup to ensure inline keyboard appears
    ...
    try:
        if data.startswith("auction_host:"):
            ...
            if action == "cancel":
                ...
                session["active"] = False
                save_session(chat_id, session)
                # --- START: Modified to delete the original start message when cancel is pressed ---
                try:
                    # Try to delete the original inline-message that had the buttons
                    if query.message:
                        try:
                            await query.message.delete()
                            # After deletion, send a simple confirmation message
                            try:
                                await _send_message(context.bot, chat_id, "Auction cancelled by admin.")
                            except:
                                pass
                        except Exception:
                            # If delete fails, fallback to editing the message (previous behavior)
                            try:
                                await query.message.edit_text("Auction cancelled by admin.")
                            except Exception:
                                await _send_message(context.bot, chat_id, "Auction cancelled by admin.")
                    else:
                        # If we don't have the original message, just send the cancellation notice
                        await _send_message(context.bot, chat_id, "Auction cancelled by admin.")
                except Exception:
                    try:
                        await _send_message(context.bot, chat_id, "Auction cancelled by admin.")
    ...
    if not bidder_team:
        for tname, aid in (session.get("assistants", {}) or {}).items():
            if aid and int(aid) == user.id:
                ...
                break
    # allow host to force finalize via /bid end
    parts = msg.text.strip().split(maxsplit=1)
