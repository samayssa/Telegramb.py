...
def build_auto_mode_choice(loaded_from_text: str):
    return InlineKeyboardMarkup([[InlineKeyboardButton("AUTO MODE", callback_data=f"auction_mode:auto:{loaded_from_text}"), InlineKeyboardButton("HOST MODE", callback_data=f"auction_mode:host:{loaded_from_text}")]])
                ...
                if teams_snapshot:
                    for tname, members in (teams_snapshot or {}).items():
                        ...
                        for l in logs_snapshot:
                            if l.get("buyer_id") and ((l.get("buyer_id") in (members or [])) or (session.get("assistants") and list(session.get("assistants").values()) and l.get("buyer_id") in list(session.get("assistants").values()))):
                                if l.get("price"):
...
async def _try_send_next_auto(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not players:
            if not (session.get("players_list") or session.get("loaded_sets") or session.get("auto_set_list")):
                return
            ...
            if run:
                players_loaded_count = run.get("players_loaded", 0)
            elif session.get("players_list"):
                players_loaded_count = len(session.get("players_list") or [])
...
async def my_team_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for l in logs:
        ...
        if buyer_team == team and l.get("price"):
            total_spent += int(l.get("price") or 0)
            team_purchases.append({"name": l.get("player_name") or "", "username": l.get("player_username") or "", "price": l.get("price")})
    total_players = len(team_purchases)
    remaining = None
    if isinstance(budget, (int, float)):
        remaining = session.get("team_budgets", {}).get(team, budget if budget is not None else 0)
    lines = []
