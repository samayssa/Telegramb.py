        ...
        if data.startswith("end_confirm:"):
            ...
            if choice == "yes":
                ...
                try:
                    ...
                    await _send_message(context.bot, chat_id, caption, parse_mode=ParseMode.HTML)
                if session.get("teams"):
                    for tname, members in (session.get("teams") or {}).items():
                        ...
                        try:
                            await context.bot.send_message(chat_id=owner_id, text=squad_caption, parse_mode=ParseMode.HTML)
                        except Exception:
                            pass
                try:
...
async def next_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if run:
        ...
        save_run(chat_id, run)
    caption = ("⟦ <b>PLAYER SET FOR AUCTION</b> ⟧\n\n"
               f"<b>Name</b>      : {name_plain}\n"
               f"<b>Username</b>  : @{(profile_username or '')}\n"
               f"<b>Role Type</b> : {player.get('role') or 'None'}\n"
               f"<b>Base Price</b>: Cr.{start_price}\n\n"
               f"<b>You have <u>{COUNTDOWN_SECONDS} seconds</u> to place your bid on this player.</b>\n"
               f"<b>Send your bid:</b> /bid <amount>\n")
    try:
        ...
        while True:
            ...
            if remaining <= 0:
                ...
                if highest:
                    ...
                    caption = ("⟦ <b>PLAYER SOLD</b> ⟧\n\n"
                               f"<b>Player</b>      : {player.get('name')}\n"
                               f"<b>Username</b>    : @{(player.get('username') or '')}\n"
                               f"<b>Role Type</b>   : {player.get('role') or 'None'}\n\n"
                               f"<b>Sold Price</b>  : Cr.{price}\n"
                               f"<b>Team</b>        : {buyer_team or '—'}\n"
                               f"<b>Buyer</b>       : {buyer_name}\n\n"
                               f"<b>The player has been successfully purchased.</b>\n")
                    try:
                    ...
                    caption = ("⟦ <b>PLAYER UNSOLD</b> ⟧\n\n"
                               f"<b>Player</b>      : {player.get('name')}\n"
                               f"<b>Username</b>    : @{(player.get('username') or '')}\n"
                               f"<b>Role Type</b>   : {player.get('role') or 'None'}\n\n"
                               f"<b>Final Status</b>: UNSOLD\n\n"
                               f"<b>No bids were received for this player.</b>\n")
                    try:
...
async def bid_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if amt < slot.get("start_price", 0):
        ...
        await msg.reply_text(f"<b>Your bid is lower than the current price.</b>\n<b>Highest bid is still held by:</b> {leading_team}.", parse_mode=ParseMode.HTML)
        return
    highest = slot.get("highest")
    if highest:
        if highest.get("user_id") == user.id:
            leading_team = _get_team_of_user(session, highest.get("user_id")) or user.first_name or ""
            await msg.reply_text(f"<b>Your bid is already registered for this player.</b>\n<b>Current top bidder remains:</b> {leading_team}", parse_mode=ParseMode.HTML)
            return
    ...
    text = ("⟦ <b>BID CONFIRMED</b> ⟧\n\n"
            f"<b>Player</b>        : { (slot.get('player') or {}).get('name') }\n"
            f"<b>Username</b>      : @{((slot.get('player') or {}).get('username') or '')}\n"
            f"<b>Role Type</b>     : {((slot.get('player') or {}).get('role') or 'None')}\n\n"
            f"<b>Current Price</b> : Cr.{amt}\n"
            f"<b>Bidder</b>        : {bidder_link}\n"
            f"<b>Team</b>          : {bidder_team}\n\n"
            f"<b>Your bid has been recorded.</b>\n"
            f"<b>Note:</b> You have <u>{COUNTDOWN_SECONDS} seconds</u> for the next bid.\n")
    try:
...
async def my_team_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for tname in user_teams:
        ...
        header = f"⟦ <b>TEAM LINE-UP ( {tname} )</b> ⟧\n\n"
        header += f"<b>Total Players Bought</b> : {total_players}\n\n<b>Players:</b>\n\n"
        for p in team_purchases:
            header += (f"<b>Player Name</b>: {p.get('name')}   |   <b>Username</b>: @{(p.get('username') or '')}  \n"
                       f"<b>Bought At</b> : Cr.{p.get('price')}\n\n")
        header += f"<b>Total Budget</b>        : Cr.{budget if budget is not None else 'Not set'}  \n"
        header += f"<b>Spent Balance</b>       : Cr.{total_spent}  \n"
        header += f"<b>Remaining Balance</b>   : Cr.{remaining if remaining is not None else 'Not set'}\n"
        parts.append(header)
    final_text = "\n\n".join(parts)
